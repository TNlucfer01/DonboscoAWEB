                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {step === 1 && (
                        <>
                            ...
                            <Button type="submit" className="w-full bg-slate-700 hover:bg-slate-800 text-white">Send OTP</Button>
                        </>
                    )}
                    {step === 2 && (
                        <>
                    ...
                    {step === 3 && (
                        <>
                            ...
                            <Button type="submit" className="w-full bg-slate-700 hover:bg-slate-800 text-white">Reset Password</Button>
                        </>
                    )}
...
// ─── Main Login Form ──────────────────────────────────────────────────────────
// Mock credentials:  principal/principal123  |  yc/yc123  |  staff/staff123
const DEMO_CREDENTIALS: Record<string, { password: string; displayName: string }> = {
    principal: { password: 'principal123', displayName: 'Principal' },
    yc: { password: 'yc123', displayName: 'Year Coordinator' },
    staff: { password: 'staff123', displayName: 'Staff Member' },
};
...
export default function Login({ onLogin }: LoginProps) {
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        if (cred && cred.password === password) {
            onLogin(username, cred.displayName);
        } else {
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                        ...
                        <Input id="username" type="text" value={username} onChange={(e) => setUsername(e.target.value)}
                            placeholder="Enter username" className="mt-1 border-slate-300" />
                    </div>
                    <div>
                    ...
                    <Button type="submit" className="w-full bg-slate-700 hover:bg-slate-800 text-white">Login</Button>
